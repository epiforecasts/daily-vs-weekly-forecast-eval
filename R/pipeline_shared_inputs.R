...
#' @title Get stan diagnostics
#' @description
#' Summarise the diagnostic information contained in a `<stanfit>` or
#' `<CmdStanFit>` object. If
#' the object is not of this class, return a data.table with NA values.
...
get_stan_diagnostics <- function(fit) {
  if (inherits(fit, "stanfit") | inherits(fit, "CmdStanFit")) {
    ...
    posterior_summary <- posterior::summarize_draws(
      fit,
      c(posterior::default_convergence_measures(), "ess_basic")
    ) |>
    ...
    diagnostics[,
      no_at_max_treedepth := sum(treedepth_data$Value == max_treedepth)
...
elapsed_time <- function(fit) {
  if (inherits(fit, "stanfit")) {
    ...
    stop("error in get elapsed_time")
  }
}
...
#'
#' @examples
ratchet_control <- function(stan_cfg) {
  if (stan_cfg$backend == "rstan") {
    within(stan_cfg, {
  ...
  } else if (stan_cfg$backend == "cmdstanr") {
    within(stan_cfg, {
      adapt_delta <- min(0.990, adapt_delta + (1 - adapt_delta) * 0.25)
    })
  } else {
